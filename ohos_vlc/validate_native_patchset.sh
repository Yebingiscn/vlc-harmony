...
grep -q 'OHCodec transparent OSD Surface attached (async display-size RGBA blend' "$WORKFLOW_FILE"
...
grep -q 'Audio low-latency queue configured' \
    "$WORK_DIR/vlc/modules/audio_output/audiounit_ohos.c"
grep -q 'OHCodec transparent OSD Surface attached (async display-size RGBA blend' \
    "$WORK_DIR/vlc/modules/video_output/ohos/display.c"
grep -q 'static void \*OsdThread' \
    "$WORK_DIR/vlc/modules/video_output/ohos/display.c"
grep -q 'QueueOsd(vd, subpic)' \
    "$WORK_DIR/vlc/modules/video_output/ohos/display.c"
grep -q 'scale_subpictures_to_display = true' \
    "$WORK_DIR/vlc/modules/video_output/ohos/display.c"
grep -q 'vd->info.scale_subpictures_to_display ||' \
    "$WORK_DIR/vlc/src/video_output/video_output.c"
grep -q '!sys->osd_visible && !sys->osd_force_upload' \
    "$WORK_DIR/vlc/modules/video_output/ohos/display.c"
